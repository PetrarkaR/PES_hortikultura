...
void formBuffer()
{
  ...
  {
    if (Comm[i] == 1)
    { // samo slejvovi koji su se odazvali
      appendBuffer("Basta:");
      ByteToStr(i, txt);
      ...
      // tj. izbaci \n\n
      // neka to samo bude na kraju, kada treba da se prikazuju podaci za narednu bastu
                                  ...
                                  unsigned int reqLength, char *canCloseTCP)
{
  ...
  { // POLL
    FlagPoll = 1;
    // getStatusID = (getRequest[6] & 0x0F) * 10 + (getRequest[7] & 0x0F);
  }
...
void interrupt()
{
  if ((PIE1.TMR1IE == 1) && (PIR1.TMR1IF == 1))
  {
    ...
    PIR1.TMR1IF = 0;
    if (brojac == 0x04)
    { // na svakih 125ms prozivka slejvova
      brojac = 0x00;
      Flag1 = 0x01;
    }
    else
  ...
  while (1)
  {
    ...
    { // 125ms
      ...
      if (Garden[SLAVE_ID].gardenSend == 0x01)
      {
        ...
        // Sanity check
        // B.J. Obavezno  pisite u komentarima sta se dobija od slejva kao odgovor
        // koliko ima bajtova, kako izgledaju
      }
