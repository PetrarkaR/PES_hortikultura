...
void ProcessInputs()
{
   ...
   if ((cntManual == 0) && (PinTaster == 1))
   {
      cntManual = DEBOUNCE_TICKS;
      ManualEvent = 1;
   }

   if ((cntManual == 0) && (PinReset == 1))
   {
      cntManual = DEBOUNCE_TICKS;
      ResetEvent = 1;
   }
...
void main()
{
   ...
   while (1)
   {
      ...
      if (UpdateLCDFlag == 1)  // 1 sekunda
      {
         ...
         // Countdown
         if (m_bWatering == 1) //(PinWatering == 1)
         {
            if (WateringSec > 0)
               WateringSec--;
...
void interrupt()
{
   ...
   if ((PIE1.RCIE) && (PIR1.RCIF))
   {
      ...
      if (ByteID == 0x00)
      {
         if (((ch & 0x0F) == GARDEN_ID) && ((ch & 0xE0) == 0xA0))  // mode code
         {
            Command = ch;
            ByteID = 0x08;
            Counter2 = 4;
         }
         else if (ch == 0x7F) //((ch & 0xE0) == 0x60)
         {
            ByteID = 0x03;
            Counter2 = 3;
         }
         else if (((ch & 0x0F) == GARDEN_ID) && ((ch & 0xE0) == 0x20))  // status code
         {
            Command = ch;
            ByteID = 0x00;
            CallFlag = 1;
