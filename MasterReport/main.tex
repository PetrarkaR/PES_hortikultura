...
Slejv uređaji su u normalnom radu preko RS-485 magistrale povezani sa Master automatom koji prikuplja informacije sa Slejv uređaja.
RS-232 veza koristi se kao jednostavnija razvojna i testna veza kada je povezan samo jedan Slejv uređaj na malom rastojanju.
...
\section{Arhitektura sistema}
  ...
  Pristup Master automatu vrši se putem HTTP protokola, kucanjem statičke adrese direktno u browser. \newline
 ...
 Automati su u slučaju kratke udaljenosti i rada sa jednim Slejvom povezani putem RS-232 veze, dok se za normalan rad sa više Slejv automata koristi RS-485 magistrala.\newline
...
\end{figure}
  ...
  CRS i Master komuniciraju po HTTP request-response modelu, dok Master i Slejv automati realizuju obostranu komunikaciju preko serijske veze.
\newpage 
\section{Realizacija komunikacionog protokola}

  \subsection{Komunikacija između Master i Slejv automata}

Komunikacija između Master i Slejv automata realizovana je preko UART modula mikrokontrolera i RS-485 drajvera u normalnom radu.
  RS-485 je izabran zato što omogućava rad više uređaja na istoj magistrali i pouzdan diferencijalni prenos na većim rastojanjima.
  RS-232 veza ostaje pogodna za razvoj i testiranje sa jednim uređajem na kratkom rastojanju.
  U realizovanom sistemu komunikacija radi na bitskoj brzini od 19200 bps, a smer rada RS-485 drajvera određuje signal \texttt{DR}.
  ...
  \subsection{Komunikacija između Master automata i Centralnog Računarskog sistema}
    ...
    Master automat podešen je da ima statičku IPv4 adresu 10.99.12.1, koja je na mreži Centralnog Računarskog Sistema.
...
\section{Hardverska Realizacija automata}
  ...
  Električna šema Master automata sastoji se od 2x16 LCD-a, RS-232, RS-485 i Ethernet interfejsa, pinova i bloka za programiranje mikrokontrolera, mikrokontrolera PIC18LF2525-I/SP, bloka za napajanje, četiri tastera povezana na \texttt{PORTB}, svaki sa svojim prekidom, i dve LED diode, od koje se jedna koristi kao znak za prolazak kroz polling svake bašte.
  ...
  Za naš sistem izabran je PIC18LF2525-I/SP u DIP28 kućištu.
  Sa svojih 25 MHz i naponom napajanja od 5 V i više je nego dovoljan za zahteve projekta.
  ...
  Da bismo programirali ovaj mikrokontroler koristimo PICkit 3 programator.
  
  Na ploči nalazimo i ENC28J60, Ethernet drajver predstavljen u SOIC-28 pakovanju, koji funkcioniše na 25 MHz na naponu od 3.3 V.

  \begin{figure}[ht]   
      \hspace{-1.8cm}\includegraphics[width=1.2\textwidth]{MasterSema.pdf}
    ...
    \begin{tabular}{|c|c|p{10cm}|}
      ...
      Port & Smer & Opis \\
      \hline
      PORTA.F5 & Izlaz & DR - Write/read kontrola RS-485 drajvera SN65176, postavlja se na 1 pri upisu. \\
      \hline
  ...
  \begin{figure}[ht]
    \hspace{-1cm}\includegraphics[width=1.1\textwidth]{MasterTopOverlay.pdf}
...
\subsection{Šema Slejv automata}
  Električna šema Slejv automata sastoji se od razvojne ploče sa mikrokontrolerom PIC16F874A u korišćenoj šemi, RS-232/RS-485 komunikacionog interfejsa, 16x2 LCD displeja, tastera za manuelno zalivanje, tastera za resetovanje lokalnog stanja, analognog ulaza za senzor protoka i indikacionih izlaza za stanje sistema, rad pumpe i alarm.
  Slejv automat je postavljen kod pumpe za zalivanje i zadužen je za lokalno upravljanje pumpom, očitavanje senzora protoka, prikaz stanja na displeju i komunikaciju sa Master automatom.

  Za Slejv automat korišćen je PIC16F874A/PIC16F877A kompatibilan razvojni sistem, jer obezbeđuje dovoljan broj digitalnih ulaza i izlaza, UART komunikaciju, Tajmer 1, A/D konvertor i paralelno povezivanje LCD displeja.
  Mikrokontroler radi na taktu od 20 MHz i napajanju od 5 V.
  Kao i kod Master automata, serijska komunikacija koristi UART modul, dok se izbor između RS-232 i RS-485 veze vrši kratkospojnicima na razvojnoj ploči.
  RS-232 je pogodan za razvoj i testiranje sa jednim uređajem na malom rastojanju, dok se u normalnom radu sistema koristi RS-485, jer omogućava povezivanje većeg broja Slejv automata na zajedničku magistralu.

  \begin{figure}[ht]
    \centering
    \IfFileExists{images/SlejvSema.pdf}
      {\includegraphics[width=1.1\textwidth]{SlejvSema.pdf}}
      {\fbox{\begin{minipage}[c][5cm][c]{0.9\textwidth}\centering
        Dodati sliku \texttt{images/SlejvSema.pdf}: električna šema Slejv automata.
      \end{minipage}}}
    \caption{Električna šema Slejv automata}
  \end{figure}
  ...
  \begin{tabular}{|m{3cm}|m{2cm}|m{8cm}|}
    ...
    \texttt{PORTC.F5} & izlaz & \texttt{DR} signal za izbor smera RS-485 drajvera \\ \hline
  ...
  \end{tabular}
  \caption{Tabela portova mikrokontrolera Slejv automata}
  \end{figure}
  \normalsize

  \begin{figure}[ht]
    \centering
    \IfFileExists{images/SlejvTopOverlay.pdf}
      {\includegraphics[width=1.1\textwidth]{SlejvTopOverlay.pdf}}
      {\fbox{\begin{minipage}[c][5cm][c]{0.9\textwidth}\centering
        Dodati sliku \texttt{images/SlejvTopOverlay.pdf}: top overlay Slejv automata.
      \end{minipage}}}
    \caption{Top overlay Slejv automata}
  \end{figure}

\clearpage

  \subsection{Prikaz sadržaja displeja Slejv automata}
  Slejv automat sadrži 16x2 LCD displej na kome se prikazuju podaci potrebni za lokalni nadzor rada pumpe.
  Za razliku od Master automata, koji prikazuje programe i status svih bašti, Slejv prikazuje samo stanje lokalne bašte: trenutno vreme, protok, alarm, manuelni režim, vreme početka programa i preostalo vreme zalivanja.

  Prvi red displeja sadrži trenutno vreme i očitanu vrednost protoka:
  \[
  ...
  Oznaka \texttt{S} označava vreme početka zadatog programa zalivanja, dok oznaka \texttt{R} označava preostalo vreme rada pumpe.
  Vrednost protoka se prikazuje kao trocifren broj iza oznake \texttt{F}.

  \begin{figure}[ht]
    \centering
    \IfFileExists{images/screenshot.png}
      {\includegraphics[width=0.7\textwidth]{screenshot}}
      {\fbox{\begin{minipage}[c][3cm][c]{0.45\textwidth}\centering
        \texttt{screenshot.png}
      \end{minipage}}}
    \caption{Format prikaza Slejv displeja}
  \end{figure}
...
\section{Realizacija firmvera Master automata}
  ...
  Kod za firmver se sastoji od funkcija za inicijalizaciju registara mikrokontrolera, inicijalizaciju globalnih promenljivih, funkcija za komunikaciju između automata i centralnog računarskog sistema, globalnog timer interrupta, funkcija za ispis na LCD i main funkcije.\newline
  ...
  \begin{figure}[ht]
    ...
    \begin{tabular}{|c|c|p{5.5cm}|}
      ...
      \hline
      SPI\_Ethernet\_HALFDUPLEX & 0 & Komunikacija sa CRS je u poludupleks režimu\\
      \hline
      SPI\_Ethernet\_FULLDUPLEX & 1 & Komunikacija sa CRS je u punom dupleksu\\
      \hline
      DR & PORTA.F5 & RS-485 kontrola smera prenosa\\
      \hline
...
\begin{figure}[ht]
  ...
  \begin{tabular}{|p{5cm}|p{5cm}|p{5cm}|}
    ...
    \hline 
    startMin & unsigned char & Minut početka programa \\
    \hline 
...
\begin{figure}[ht]
    ...
    \begin{tabular}{|c|c|c|p{5cm}|}
      ...
      \hline
      FlagPoll & unsigned char & 0x00 & Flag da su svi Slejv automati prozvani \\
      \hline
      updateLCDFlag & unsigned char & 0x00 & Flag za osvežavanje LCD-a \\
      \hline
      SLAVE\_ID & unsigned char & 0x0F & Brojač trenutno prozivanog Slejv automata; početna vrednost 0x0F omogućava da prva inkrementacija započne od Slejva 0 \\
      \hline
      ...
      \hline
       Status1[ ] & unsigned char & 0x00 & Status bajt koji svaki Slejv šalje Masteru na prozivku \\
      \hline
       ...
       \hline
        btnCnt &  unsigned char  & 0x00 & Brojač za debounce tastera \\
       \hline
...
\begin{centering}
  \begin{tabular}{|p{5cm}|p{2cm}|p{8cm}|}
    ...
    \hline 
     ...
     \hline
     unsigned int SPI\_Ethernet\_UserTCP(unsigned char *remoteHost, unsigned int remotePort, unsigned int localPort, unsigned int reqLength, char *canCloseTCP) & u\_int & Obrađuju se zahtevi koji pristižu iz WEB pretraživača i izvršava se slanje podataka preko Etherneta \\
     \hline
     ...
     \hline
     lcdDisplayUchar(unsigned char row, unsigned char col, unsigned char value) & void & Formatiranje prikaza unsigned chara \\
     \hline 
...
\end{minted}
\textbf{Funkcija main} \newline
Glavna funkcija u kojoj se najpre pozivaju funkcije za inicijalizaciju, nakon kojih sledi glavna beskonačna petlja iz koje se izlazi isključivo po prekidu tajmera, to jest na svakih 25 ms.
  ...
  while (1) {
    ...
    if (Flag1 == 0x01) { // 125ms
      ...
      if (Garden[SLAVE_ID].gardenSend == 0x01) {
        ...
        // RTC se šalje broadcast porukom svim baštama.
        transmit(hours);
        transmit(minutes);
        transmit(seconds);
        DR = 0;
...
\section{Realizacija firmvera Slejv automata}
  Firmver Slejv automata napisan je za mikroC kompajler i korišćeni PIC16F874A/PIC16F877A kompatibilni kontroler.
  ...
  Kod za firmver se sastoji od funkcija za inicijalizaciju registara mikrokontrolera, inicijalizaciju globalnih promenljivih, obrade prekida, obrade tastera, komunikacije sa Master automatom, formiranja statusnog bajta, ispisa na LCD i glavne funkcije.
  U nastavku su date tabele makroa, pinova, promenljivih i funkcija, a zatim i objašnjenja najvažnijih funkcionalnih celina.
  ...
  \begin{tabular}{|m{3.5cm}|m{3cm}|m{6.5cm}|}
    ...
    \texttt{DR} & \texttt{PORTC.F5} & Upravljanje smerom RS-485 drajvera \\ \hline
    \texttt{DEBOUNCE\_TICKS} & \texttt{10} & Broj tajmerskih intervala za zaštitu od treperenja tastera \\ \hline
  \end{tabular}
  \caption{Tabela makroa i pinova Slejv firmvera}
  \end{figure}
  \normalsize

  Data je tabela najvažnijih globalnih promenljivih korišćenih u Slejv firmveru.

  \begin{figure}[ht]
  \centering
  \small
  \begin{tabular}{|m{3.6cm}|m{3cm}|m{6.8cm}|}
    \hline
    ...
    \texttt{ByteID} & \texttt{unsigned char} & Stanje prijema poruke preko UART-a \\ \hline
    ...
    \texttt{Tmp\_ProgStartHour}, \texttt{Tmp\_ProgStartMin} & \texttt{unsigned char} & Privremeno primljene vrednosti vremena početka programa \\ \hline
    \texttt{time\_left\_high}, \texttt{time\_left\_low} & \texttt{unsigned char} & Primljene cifre trajanja zalivanja \\ \hline
  ...
  \end{tabular}
  \caption{Tabela globalnih promenljivih Slejv firmvera}
  \end{figure}
  \normalsize
  ...
  Ispod je tabela svih funkcija koje nose rad Slejv automata. Kao i kod Master firmvera, posle tabele su izdvojene najvažnije funkcije i delovi koda.
  ...
  \begin{tabular}{|m{3.8cm}|m{3cm}|m{6.8cm}|}
    ...
    \texttt{toBcd()} & \texttt{unsigned char} & Konvertuje decimalnu vrednost u BCD format za poređenje vremena \\ \hline
    \texttt{ConvertTime()} & \texttt{void} & Razdvaja primljenu decimalnu vrednost vremena na desetice i jedinice \\ \hline
    \texttt{LcdOut2()}, \texttt{LcdOut3()} & \texttt{void} & Formatiraju dvocifrene i trocifrene vrednosti za LCD \\ \hline
    \texttt{LcdOutDuration()} & \texttt{void} & Prikazuje preostalo vreme zalivanja u formatu \texttt{MM:SS} \\ \hline
    ...
    \texttt{main()} & \texttt{void} & Glavna funkcija programa u kojoj se obrađuju događaji i upravlja pumpom \\ \hline
  \end{tabular}
  \caption{Tabela funkcija Slejv firmvera}
  \end{figure}
  \normalsize
  ...
  \subsection{Detaljan opis najvažnijih funkcija sa segmentima koda}
  \textbf{Funkcija init\_variables} služi za inicijalizaciju varijabli Slejv automata na podrazumevane vrednosti.
  U početnom stanju brišu se flegovi za komunikaciju i tastere, vreme se postavlja na nulu, pumpa i alarm se gase, a sistemska indikacija se uključuje.
  \begin{minted}{C}
void init_variables()
{
   GARDEN_ID = 0x00;
  ...
  \textbf{Funkcija init} podešava hardver Slejv automata.
  Port A se delimično koristi za analogni ulaz senzora protoka, a delimično za izlazne indikacije.
  Port B je podešen kao ulaz za tastere, port C sadrži UART linije, LCD kontrolne linije i \texttt{DR} signal RS-485 drajvera, dok se port D koristi i za ID uređaja i za podatke LCD displeja.
...
void init()
{
  ...
  \textbf{Funkcija ProcessInputs} obrađuje taster za manuelno zalivanje i taster za resetovanje lokalnog stanja.
  Oba tastera imaju debounce zaštitu preko brojača koji se smanjuju na svakom prekidu Tajmera 1.
  Kada se registruje validan pritisak, funkcija ne izvršava kompletnu akciju odmah, već postavlja odgovarajući događaj koji kasnije obrađuje glavna petlja.
  \begin{minted}{C}
void ProcessInputs()
{
   if (cntManual > 0)
  ...
  \textbf{Funkcija buildStatusByte} formira statusni bajt koji Slejv vraća Master automatu.
  Redosled bita odgovara protokolu opisanom u poglavlju komunikacije: \texttt{S}, \texttt{W}, \texttt{A} i \texttt{M} zauzimaju četiri najviša bita statusnog bajta.
  \begin{minted}{C}
unsigned char buildStatusByte()
{
  ...
  \textbf{Funkcija interrupt} ima dve glavne grane.
...
void interrupt()
{
  ...
  \textbf{Funkcija main} najpre poziva inicijalizacione funkcije za registre, promenljive i LCD, a zatim ulazi u beskonačnu petlju.
  ...
  \begin{gathered}
    ...
    \texttt{Seconds == 0}\\
    \texttt{Hours == ProgStartHour}\\
    \texttt{Minutes == ProgStartMin}
  \end{gathered}
  \]
  Kako se start proverava samo kada su sekunde jednake nuli, isti dnevni program se pokreće jednom u zadatom minutu.
...
else
{
  ...
  \textbf{Funkcija UpdateLCD} osvežava oba reda displeja.
  Prvi red prikazuje tekuće vreme i trenutnu vrednost protoka.
  Drugi red prikazuje alarm, manuelni režim, aktivno vreme početka programa i preostalo vreme do gašenja pumpe.
...
void UpdateLCD()
{
   ...
   Lcd_Out(2, 3, "S:");
   Lcd_Chr(2, 4, ((ProgStartHour >> 4) & 0x0F) + '0');
   Lcd_Chr(2, 5, (ProgStartHour & 0x0F) + '0');
   Lcd_Out(2, 6, ":");
   Lcd_Chr(2, 7, ((ProgStartMin >> 4) & 0x0F) + '0');
   Lcd_Chr(2, 8, (ProgStartMin & 0x0F) + '0');
   Lcd_Out(2, 10, " R");
...
\section{Postupak verifikacije i vrednovanja koda }
 \subsection{Rezultat rada sistema}
  Verifikacija sistema vrši se povezivanjem Master i Slejv razvojne ploče preko serijske veze i učitavanjem odgovarajućih firmvera u mikrokontrolere.
  U normalnom radu predviđena je RS-485 magistrala, dok je RS-232 korišćen kao razvojna i testna veza pri radu sa jednim Slejv automatom.
  ...
  Master proziva sledeći Slejv na svakih 125 ms, pa se u konfiguraciji sa svih 16 Slejv automata status pojedinačne bašte osvežava približno na svake 2 s.
  ...
  Očitavanje se u firmveru svodi na osmobitnu vrednost čitanjem registra \texttt{ADRESH}, iako A/D konvertor hardverski ima veću rezoluciju.
  U trenutnoj verziji firmvera granice protoka su definisane u programu kao početne vrednosti \texttt{FlowMin = 20} i \texttt{FlowMax = 60}, dok se daljinsko podešavanje granica može dodati proširenjem protokola na isti način kao što je realizovano podešavanje programa zalivanja.
  ...
  Može da programira Slejv automate UART protokolom, preko RS-485 magistrale u normalnom radu ili RS-232 porta za kratke distance, da preko istog protokola prihvata podatke, čuva ih i direktno prikazuje u internet pretraživač u vidu stringa.
...
\section{Detaljno uputstvo za upotrebu}
  ...
  Nakon toga se Slejv povezuje sa Master automatom preko RS-485 magistrale, odnosno preko RS-232 konektora kada se ispituje jedan uređaj na kratkom rastojanju, i uključuje se napajanje razvojne ploče.
...
/*
 * ========================================================
 *RTC broadcast -> 0x7F + 3 Bytes HH|MM|SS
 *Biranje moda 101|x|idid| -> 0xA0 + 4 Bytes HH|MM|SS|SS
 * ========================================================
 */
